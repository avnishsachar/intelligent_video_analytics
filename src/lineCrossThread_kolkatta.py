## Line Cross code deployed at Webel ##
...
class LineCrossing(threading.Thread):
    ...
    def run(self):
        ...
        while count<10:         #Try restrating for max 10 times
...
def detectPeople(name, rtsplink, line1, line2, region, case, slope, livestream, threshold):
    ...
    backSub.setVarThreshold(50)
    #backSub.setBackgroundRatio(0.65)
    backSub.setHistory(2000)
    backSub.setVarMax(120)
    ...
    while True:
        ...
        countFrame += 1
        check_location(os.path.join(os.getcwd(),"LineCrossing",str(datetime.now()).split(' ')[0],name))
        if countFrame%4==0:  # to skip frames
            if ret == True:
                ...
                thresh = backSub.apply(frame,learningRate=0.5)
...
def lineCross(csvFile):
    ...
    while True:
        ...
        if liveStream_name != None and  len(liveStream_frame.shape)>0:
        ...
        if eventStream_path != None and event_occured == True:
            ...
            for i in range(1):             #change no to change the no of time pop up should blink
                cv2.imshow("Warning at %s!!"%eventStream_name, img)
                cv2.moveWindow("Warning at %s!!"%eventStream_name, 300,300)
                cv2.waitKey(1000)
            ...
            while j<1:                  #set the duration till which event frame needs to be displayed
...
lineCross(os.getcwd()+"/LineCrossing/camera_details.csv")   ##comment when running with main GUI
